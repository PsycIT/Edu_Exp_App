...
from feedback_page import SecondWindowCls # 3rd page 포함한 정상작동 x 버전 (confidence 받는 부분 포함, 멈추는 코드) # 45line도
...
class WindowCls(QMainWindow, form_class) :
    def __init__(self) :
        ...
        self.df = pd.DataFrame([['EXP_START', self.mainStartTs]],
                               index=[self.expInfoDict['idxCnt']], columns=['status', 'ts'])
    ...
    def startBtn_clicked(self):
        ...
        self.hide()
        self.feedback_page = SecondWindowCls(self.expInfoDict)
